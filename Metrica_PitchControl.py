...
def check_offsides(
    ...
    defending_GK_id = GK_numbers[1] if attacking_players[
        0].teamname == 'Home' else GK_numbers[0]
    ...
    second_deepest_defender_x = sorted(
        [defending_half * p.position[0] for p in defending_players],
        reverse=True
    )[1]
    # define offside line as being the maximum of second_deepest_defender_x, ball position and half-way line
    offside_line = max(
        second_deepest_defender_x, defending_half * ball_position[0], 0.0
    ) + tol
...
class player(object):
    ...
    def probability_intercept_ball(self, T):
        ...
        f = 1 / (
            1. + np.exp(
                -np.pi / np.sqrt(3.0) / self.tti_sigma *
                (T - self.time_to_intercept)
            )
        )
        assert 0 <= f <= 1, f'Probability to intercept ({f}) should be between 0 and 1'
        return f
...
def default_model_params(time_to_control_veto=3):
    ...
    params[
        'max_player_accel'
    ] = 7.  # maximum player acceleration m/s/s, not used in this implementation
    ...
    params[
        'kappa_def'
    ] = 1.  # kappa parameter in Spearman 2018 (=1.72 in the paper) that gives the advantage defending players to control ball, I have set to 1 so that home & away players have same ball control probability
    params['lambda_att'] = 4.3  # ball control parameter for attacking team
    params['lambda_def'] = 4.3 * params[
        'kappa_def']  # ball control parameter for defending team
    params['lambda_gk'] = params[
        'lambda_def'
    ] * 3.0  # make goal keepers must quicker to control ball (because they can catch it)
...
def generate_pitch_control_for_event(
    ...
    assert 1 - checksum < params['model_converge_tol'
                                ], "Checksum failed: %1.3f" % (1 - checksum)
    return PPCFa, xgrid, ygrid


def calculate_pitch_control_at_target(
    target_position, attacking_players, defending_players, ball_start_pos,
    params
):
    ...
    if tau_min_att - max(ball_travel_time,
        ...
        while 1 - ptot > params['model_converge_tol'] and i < dT_array.size:
            ...
            for player in attacking_players:
                ...
                PPCFatt[
                    i
                ] += player.PPCF  # add to sum over players in the attacking team (remembering array element is zero at the start of each integration iteration)
